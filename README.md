...
- **Marcado MEDIR y autocarga al Excel** (durante la descarga/procesamiento de etiquetas): si esta configurado el Excel de medidas:
  ...
  3. **Datos de embalaje en la etiqueta**: cada etiqueta individual (no CARROS) con SKU numerico **de pedido de 1 unidad** lleva, en el margen superior derecho, entre 1 y 3 lineas segun lo cargado en el Excel (si `ESTANDARIZADO` dice `NO`, la unica linea es `NO ESTANDARIZADO`, mas grande y en negrita). Las ordenes de 2+ unidades no la llevan: el operario no esta embalando un producto suelto, asi que la instruccion de envase no le sirve. Aplica igual a las etiquetas turbo, que se tratan como cualquier otra zona.
