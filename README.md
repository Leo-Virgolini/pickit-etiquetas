...
- **Marcado MEDIR y autocarga al Excel** (durante la descarga/procesamiento de etiquetas): si esta configurado el Excel de medidas:
  ...
  4. **Columna Estandarizado en la tabla**: la tabla de etiquetas descargadas muestra **una fila por etiqueta** -cada fila es un paquete, con su numero impreso, su venta y las unidades que van adentro- con las columnas #, Orden (el numero que ML imprime como `Pack ID:` o `Venta ID:`, que sale de la API o del propio ZPL al procesar un archivo local), Zona, SKU, Producto, Detalles y Cantidad. La agrupacion por SKU se mantiene para el ZPL, que es la que define el orden de impresion. La columna Estandarizado muestra el valor de la columna `ESTANDARIZADO` del Excel: `✓ SI`, `✘ NO` (fondo rosa palido) o `—` cuando no aplica (carros y SKU no numericos, o modulo de medidas apagado). Informa sobre el **SKU** y no sobre una etiqueta puntual, asi que no mira la cantidad: una fila puede decir `✘ NO` aunque ninguna de sus etiquetas lleve el aviso impreso por ser de 2+ unidades. Permite ver el estado del lote sin depender del dialogo final.
