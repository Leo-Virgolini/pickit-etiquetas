...
- **Excel de medidas ML** (opcional, se activa con checkbox): base "madre" de medidas de embalaje por SKU, usada para marcar etiquetas pendientes de medir y para cargar las dimensiones de paquete en ML (atributos `SELLER_PACKAGE_*`) y para indicar en la etiqueta que embalaje usar. 13 columnas en fila 1:
  ...
  - Lo mismo con `EMBALAJE` y la hoja `EMBALAJES`: se crean al leer el archivo si faltan, y solo entonces se reescribe (la lectura detecta si falta algo, asi que en regimen normal no se vuelve a abrir en modo escritura). Solo se migra una hoja que tenga columna `SKU`: las hojas propias del usuario no se tocan. En un archivo existente `EMBALAJE` se **inserta antes de `ERROR`**, desplazando un lugar a la derecha esa columna y cualquier otra que el usuario tenga despues (con su contenido y formato). Las formulas se mueven con su texto original: si tenias una formula que apunta a la columna `ERROR` o a una posterior, revisa su referencia despues de la migracion. Si ya existe se reusa este donde este: ambas columnas se ubican por header, no por indice.
