...
- **Marcado MEDIR y autocarga al Excel** (durante la descarga/procesamiento de etiquetas): si esta configurado el Excel de medidas:
  ...
  3. **Datos de embalaje en la etiqueta**: cada etiqueta individual (no CARROS) con SKU numerico lleva, en el margen superior derecho, entre 1 y 4 lineas segun lo cargado en el Excel. Aplica igual a las etiquetas turbo, que se tratan como cualquier otra zona.
     ...
     | `ESTANDARIZADO` en `SI` pero sin ninguna de las tres columnas de datos | `SIN DATOS DE EMBALAJE`, y es la unica linea |
     ...
     En un pedido de 2+ unidades el operario no esta embalando un producto suelto, asi que el envase es orientativo: sale encabezado por `REFERENCIA`. Los **avisos no dependen de la cantidad**: salen igual en las etiquetas de 1 unidad y en las de varias, y siempre sin el encabezado, que rotula datos de embalaje y en un aviso no hay ninguno. Los dos van en un recuadro negro con el texto en blanco y centrado, para que frenen al operario; el cuerpo lo decide el largo del texto (`NO ESTANDARIZADO` a 38, `SIN DATOS DE EMBALAJE` a 32) porque `^FB` no descarta lo que no entra sino que lo reimprime encima y queda una mancha. La linea de observaciones se queda con el alto libre que sobra, asi que un texto largo sigue en las lineas de abajo en vez de imprimirse encima de si mismo. El bloque avanza de a 26 —el alto de la fuente, que es el mismo interlineado que `^FB` usa entre las filas de una linea—: con un paso mayor el aire de mas se lo come la ultima linea, que es la que crece. Los rotulos (`ENVASE:`, `ROLLO:`, `OBS:`) van en negrita. `REFERENCIA` va en negrita, subrayado y en la fuente residual B de Zebra (`^ABN,22,14`), de trazo mas cuadrado que el resto, para que se lea como un rotulo y no como un dato mas. ZPL no tiene subrayado como atributo: es una linea dibujada, y se puede calcular exacta porque esa fuente es monoespaciada. Las lineas que no son la ultima se cortan con `...` si no entran, y las palabras muy largas (codigos, URLs) se parten para que no quede el rotulo solo arriba. Para hacerles lugar se elimina el texto de ML "Recorta esta parte de la etiqueta...", que ocupa esa franja. Si ML cambiara ese texto y no se encontrara, queda una advertencia en el log y los textos se encimarian.
  4. **Columna Estandarizado en la tabla**: la tabla de etiquetas descargadas muestra **una fila por etiqueta** -cada fila es un paquete, con su numero impreso, su venta y las unidades que van adentro- con las columnas #, Orden (el numero que ML imprime como `Pack ID:` o `Venta ID:`, que sale de la API o del propio ZPL al procesar un archivo local), Zona, SKU, Producto, Detalles y Cantidad. La agrupacion por SKU se mantiene para el ZPL, que es la que define el orden de impresion. La columna Estandarizado muestra `✓ SI`, `✘ NO` (fondo rosa palido), `⚠ SIN DATOS` (fondo ambar: la formula dice que si pero no hay ninguna columna de embalaje cargada) o `—` cuando no aplica (carros y SKU no numericos, o modulo de medidas apagado). El valor sale de **lo que el renderer imprime** y no de releer la columna del Excel por separado, para que la tabla y el papel no puedan decir cosas distintas. Informa sobre el **SKU** y no sobre una etiqueta puntual, pero como los avisos ya no dependen de la cantidad, la fila y su etiqueta siempre coinciden. Permite ver el estado del lote sin depender del dialogo final.
  5. **Mensaje al finalizar**: al terminar la descarga se abre un dialogo scrollable con cuantos SKU nuevos se agregaron al Excel —son las filas que hay que completar con el envase— y **dos listados separados**, porque no se arreglan igual: los SKU **sin estandarizar** (los que tienen `NO` en la columna `ESTANDARIZADO` y los que todavia no figuran en el Excel, a los que les falta la decision) y los SKU **estandarizados pero sin datos de embalaje cargados** (a los que les falta completar las columnas). Se listan los que efectivamente salieron con el aviso impreso, que ahora son todos los afectados: la cantidad de la etiqueta ya no los excluye.
