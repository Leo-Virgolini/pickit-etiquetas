...
- **Marcado MEDIR y autocarga al Excel** (durante la descarga/procesamiento de etiquetas): si esta configurado el Excel de medidas:
  ...
  3. **Datos de embalaje en la etiqueta**: cada etiqueta individual (no CARROS) con SKU numerico lleva, en el margen superior derecho, entre 1 y 3 lineas segun lo cargado en el Excel (la primera esta siempre: si no hay ni caja ni bolsa dice `NO ESTANDARIZADO`, mas grande y en negrita, y esa es la unica linea):
